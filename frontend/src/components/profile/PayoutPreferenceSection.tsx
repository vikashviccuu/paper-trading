...
export default function PayoutPreferenceSection({ profile, onSaved }: { profile: UserProfile; onSaved: () => void }) {
  ...
  return (
    <PCard title="Payout Preference" icon="💸" subtitle="How contest prize money is paid out to you">
      <div style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 16, padding: "12px 14px", background: "var(--bg-elevated)", borderRadius: 10, border: "1px solid var(--border)", boxShadow: "var(--card-shadow)" }}>
        ℹ️ Applies to <strong style={{ color: "var(--text-primary)" }}>future prize awards only</strong> — never retroactively changes an already-computed award.
      </div>
        ...
        {options.map((o) => {
          ...
          return (
            <div key={o.value} onClick={() => !busy && save(o.value)} style={{
              ...
              background: sel ? "var(--blue-bg)" : "var(--bg-elevated)",
              boxShadow: sel ? "0 4px 16px var(--accent-glow)" : "var(--card-shadow)",
              transition: "all 0.2s ease", opacity: busy ? 0.7 : 1,
            }}>
              <div style={{ width: 44, height: 44, borderRadius: 10, background: sel ? "var(--blue-bg)" : "var(--bg-surface)", border: `1px solid ${sel ? "var(--blue-border)" : "var(--border)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>{o.icon}</div>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
                  <span style={{ fontWeight: 700, fontSize: 14, color: sel ? "var(--text-primary)" : "var(--text-secondary)" }}>{o.title}</span>
              ...
              <div style={{ width: 22, height: 22, borderRadius: "50%", border: `2px solid ${sel ? "var(--accent)" : "var(--border)"}`, background: sel ? "var(--accent)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2, transition: "all 0.15s" }}>
